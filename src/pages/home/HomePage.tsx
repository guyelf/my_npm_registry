...
export default function HomePage() {
  ...
  const renderedPackages = featuredPackages.map((p) => {
    return (
      <div
        key={p.name}
        className='flex flex-col justify-between gap-3 border rounded'
      >
  ...
  return (
    <div className='container py-12 space-y-8'>
      <div className='space-y-6 text-center'>
        <h1 className='text-6xl font-bold'>My NPM Registry</h1>
        <p className='mx-auto max-w-[600px] text-gray-500'>
          My package manager for JavaScript. Search and view packages.
        </p>
      </div>
